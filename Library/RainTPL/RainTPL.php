...
class RainTPL{
	...
	// -------------------------
		...
		/**
		 * Cache directory. Is the directory where RainTPL will compile the template and saveAdmin the cache
		 *
		 * @var string
		 */
	...
	function draw( $tpl_name, $return_string = false ){
        ...
        // rain get the output buffer to saveAdmin the output in the cache or to return it as string
            ...
            // saveAdmin the output in the cache
            if( $this->cache )
	...
	protected function check_template( $tpl_name ){
		...
		if( !isset($this->tpl['checked']) ){
            ...
            $this->tpl['tpl_filename']          = $_SERVER['DOCUMENT_ROOT'] .DIRECTORY_SEPARATOR . $tpl_dir . $tpl_basename . '.' . self::$tpl_ext;	// template filename
        ...
        $loop_level = 0;
	 	...
	 	while( $html = array_shift( $parsed_code ) ){
			...
			elseif( preg_match( '/\{loop(?: name){0,1}="\${0,1}([^"]*)"\}/', $html, $code ) ){
				...
				//loop code
				$compiled_code .=  "<?php $counter=-1; if( isset($var) && $var instanceof Iterator || is_array($var) && sizeof($var) ) foreach( $var as $key => $value ){ $counter++; ?>";
