<?php
/**
 * Файл содержит класс модель, управляющая необходимыми ресурсами для найма юнитов-артистов.
 *
...
/**
 * Класс модель, управляющая необходимыми ресурсами для найма юнитов-артистов.
 *
 * @author vetalrakitin  <vetalrakitin@gmail.com>
 * @version 1.0.0
 * @package unit
 */ 
class unit_ArtistResourceValue extends Mapper {

	const TABLE_NAME = 'units_artists_resource_value';

    /**
     * Получение экземпляра сущности.
     *
     * @param string $className
     * @return unit_ArtistResourceValue
     */
    ...
    /**
     * Запрос на поиск необходимых ресурсов для найма юнитов-артистов
     * 
    ...
    public function findAllById($idUnit, $idPersonageBuilding)
    {
        $sql = "
			SELECT 
				...
				`V`.`artist_value_resource` as `required`,
    ...
    /**
     * Списание ресурсов за найм юнитов-аристов
     * 
     * @param $idUnit - ИД юнита
    ...
    public function writeOffResourceForHiring($unitId, $unitCount, $idPersonage, $idPersonageCity)
    {
		$sql = "
			...
			SET 
				PRS.`personage_resource_value` = PRS.`personage_resource_value` - URV.`artist_value_resource` * '%3\$d'
			WHERE 
				PRS.`id_personage` = '%4\$d' AND
				(PRS.`personages_cities_id` = '%5\$d' 
				OR PRS.`personages_cities_id` IS NULL) AND
				URV.`id_units_artists` = '%6\$d'";
    ...
    /**
     * Проверка наличия необходимых ресурсов для найма юнитов-аристов
     * 
     * @param $idUnit - ИД юнита
     * @param $unitCount - Количество юнитов
     * @param $idPersonage - ИД персонажа
    ...
    public function checkAvailabilityResources($unitId, $unitCount, $idPersonage, $idPersonageCity)
    {
		...
		$sql = "
			SELECT 
				...
				URV.`artist_value_resource` *  '%3\$d' `require`
			FROM 
				`%1\$s` PRS
			INNER JOIN 
				`%2\$s` URV  ON PRS.`resource_id` = URV.`resource_id`
