<?php
/**
 * Файл содержит класс модель, управляющая характеристиками юнитов-артистов.
 *
...
/**
 * Класс модель, управляющая характеристиками юнитов-артистов.
 *
 * @author vetalrakitin  <vetalrakitin@gmail.com>
 * @version 1.0.0
 * @package unit
 */ 
class unit_Artist extends Mapper {

	const TABLE_NAME = 'units_artists';
    ...
    /**
     * Запрос на поиск юнитов-артистов
     * 
    ...
    {
        $sql = "SELECT 
				`id_units_artists` as `id_unit`,
				`name_units_artists` as `name_unit`, 
				(SELECT ifnull(SUM(`count`), '0')
				FROM `%2\$s` 
				WHERE `id_building_personage` = '%5\$d' 
				AND `unit_id` = `id_units_artists`) as `hired`,
				'1' as `available`,
				'artist' as `unit_type`,
				IF (`level_building` <= 
   ...
   /**
     * Проверка доступности юнитов-артистов для найма
     * 
    ...
    public function findAllUnitsInQueryByIdPersonageBuilding($idPersonageBuilding)
    {
		$sql = "
			SELECT 
				...
				`UK`.`name_units_artists` as `name_unit`,
				`PU`.`count` as `count_unit`,
				`PU`.`finish_time_rent` as `finish_time`
			FROM `%1\$s` as PU
			LEFT JOIN `%4\$s` as UK ON `UK`.`id_units_artists` = `PU`.`unit_id`
	...
	/**
	 * Найм юнита-артиста, по ИД здания персонажа, ИД юнита
	 * 
	 * @param $idPersonageBuilding - ИД здания персонажа
	 * @param $unitId - ИД юнита
	 * @param $unitCount - Количество юнитов
